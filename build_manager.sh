...
# Сохранение аргументов при ручном запуске
if [ "$#" -eq 2 ]; then
...
else echo "Ошибка: Нет токенов! Использование: bash build_manager.sh <TOKEN> <CHAT_ID>"; exit 1; fi
...
if [ ! -f "$STATE_FILE" ]; then
    ...
    # Ставим python3-requests через apt, чтобы обойти PEP 668 (externally-managed-environment)
    # Ставим cron, так как он нам критически важен для автостарта
    sudo apt update
    sudo apt install -y git aria2 python3 python3-pip python3-requests tmux systemd-zram-generator curl cron
    ...
    # Гарантируем, что служба cron включена и запустится после ребута
    sudo systemctl enable cron
    sudo systemctl start cron

    # Добавляем скрипт в автозагрузку (с задержкой 30 секунд на поднятие сети)
    # Создается сессия tmux с именем 'mysetup'
    (crontab -l 2>/dev/null || true; echo "@reboot sleep 30 && tmux new-session -d -s mysetup 'bash $DIR/build_manager.sh >> $LOG_FILE 2>&1'") | crontab -

    echo "STAGE_2" > "$STATE_FILE"
    
    echo "Перезагружаю сервер... Скрипт продолжит работу в фоне (сессия tmux 'mysetup')"
    sudo reboot
else
    echo "=== ЭТАП 2: Сборка ==="
    # Удаляем себя из автозагрузки, чтобы избежать цикла
    (crontab -l 2>/dev/null || true) | grep -v 'build_manager.sh' | crontab -
    rm -f "$STATE_FILE"
    touch "$LOG_FILE"

    # 1. ЗАПУСК ДЕМОНА В ФОНЕ (Высший приоритет)
    ...
    # 5. ЖДЕМ ПОДТВЕРЖДЕНИЯ (Заморозка скрипта)
    echo "Ожидание подтверждения от пользователя через TG/Web..."
