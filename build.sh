...
# Строгий режим: останавливает скрипт при любой неожиданной ошибке
set -eEu -o pipefail

# Если возникает ошибка, отправляем её в Web UI
...
if [ ! -f "$STATE_FILE" ]; then
    ...
    # Гарантируем работу cron для автозапуска
    sudo systemctl enable cron
    sudo systemctl start cron

    # Добавляем скрипт в автозагрузку с переходом в правильную папку
    (crontab -l 2>/dev/null | grep -v 'build.sh' || true; echo "@reboot sleep 30 && cd $DIR && /usr/bin/tmux new-session -d -s evobuild '/bin/bash $DIR/build.sh >> $LOG_FILE 2>&1'") | crontab -
    ...
    echo "=== ЭТАП 2: Сборка ==="
    # Чистим автозагрузку, чтобы не ребутнуться снова
    crontab -l 2>/dev/null | grep -v 'build.sh' | crontab - || true
    rm -f "$STATE_FILE"
    touch "$LOG_FILE"

    # 1. ЗАПУСК ДЕМОНА WEB-UI
    pkill -f web_daemon.py || true
    ...
    # Автоподтверждение всех промптов repo
